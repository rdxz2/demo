...
GSHEET_SHEET = 'Saving'
GSHEET_RANGE = 'G6:O100'
BQ_TABLE_ID_PREFIX = 'gsheet__'
BQ_TABLE_ID = 'portfolio'
BQ_TABLE_COLS = {
    'mob': 'STRING',
    'date': 'DATE',
    'saver': 'STRING',
...
def fetch(sheet_id: str) -> str:
    ...
    for i in range(len(rows)):
        ...
        for col in [
            ...
            if row_length > col:
                rows[i][col] = float(rows[i][col].replace(',', ''))

        # Add missing columns
        for _ in range(cols_length - row_length):
            rows[i].append(None)
